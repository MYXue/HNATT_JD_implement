...
def activations():
	...
	if request.method == 'GET':
		...
		print ("text from app:",text)
		ntext = text_util.normalize_ch(text)
		print ('ntext from app:',ntext)

		global graph
		with graph.as_default():
			activation_maps = h.activation_maps(text, websafe=True)
			print ("activation_maps from app:",activation_maps)
			preds = h.predict([ntext])[0]
			# print ("preds from app:",preds)
